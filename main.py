...
def append_chunks_to_db(chunks):
    batch = next_batch_id()
    for t in chunks:
        c.execute("INSERT INTO chunks(text,batch_id,added_at) VALUES (?,?,?)", (t, batch, now_str()))
    conn.commit()
    return batch
...
def delete_last_batch():
    ...
    if not last:
        return 0
    c.execute("DELETE FROM chunks WHERE batch_id=?", (last,))
    conn.commit()
    return last
...
# RUN LOOP
# ---------------------------
print("🚀 JusticeAI bot is running on Render...")
while True:
    try:
